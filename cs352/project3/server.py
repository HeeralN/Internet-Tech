...
# Error page for bad cookies
bad_cookie_page = "<h1>Bad cookie! Try again</h1>" + login_form
# Successful logout
...
# Cookies database {cookie_val: username}
cookies = dict()

# Password database {username: (password, secret)}
data = dict()
...
while True:
    ...
    username = password = action = token = None
    headers_to_send = ''

    # Parse header for cookie
    headers = headers.split('\r\n')
    print(headers)
    for field in headers:
        param = field.split(':')
        if param[0] == 'Cookie':
            token = param[1].split('=')[1]

    # Parse body for username, password, or action
    ...
    if action == 'logout':
    ...
    elif token is not None: # Header contains a cookie
        if token in cookies: # Valid cookie
            html_content_to_send = success_page + data[cookies[token]][1]  # success page + secret
        else: # Invalid cookie
            html_content_to_send = bad_cookie_page # Retry login
    elif username == None and password == None:  # Default login
        ...
        cookie_val = random.getrandbits(64)
        cookies[str(cookie_val)] = username
        headers_to_send = 'Set-Cookie: token=' + str(cookie_val) + '\r\n'
